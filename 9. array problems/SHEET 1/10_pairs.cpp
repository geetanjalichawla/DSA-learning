...
#include<algorithm>
#include<vector>
vector<vector<int>> pairSumBootforce(vector<int> &arr, int s){
   // Write your code here
	sort(arr.begin(),arr.end());// no need in case of normal this is for 
...
void pairSum(vector<int> &arr, int s){
   // Write your code here
	sort(arr.begin(),arr.end());// need in case  
	
	int i = 0;
    int j = arr.size()-1;
    while (i<j)
    {
        if(arr[i]+arr[j] == s) {cout<<arr[i]<<" "<<arr[j]<<endl; i++;}
